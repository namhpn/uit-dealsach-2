...
| KI-0015 | 2026-06-01 | T0026 | Backend test baseline / seeded scenarios | Full backend PHPUnit currently fails at `DealSachDomainDatabaseTest::testSeedScenarioCoverage` because `countLatestStaleOffers()` returns `0`, below the expected `>= 2` stale-offer scenarios. T0026 reproduced the failure after fresh migrate+seed; fixing seeded stale-window coverage is outside the wishlist visual-refinement scope. | Medium | Add a focused seed/test-baseline ticket to stabilize stale-offer scenario coverage in `DealSachDemoSeeder` and align `DealSachDomainDatabaseTest` expectations. |
| KI-0015 | 2026-06-01 | T0025 | Backend test baseline | Full backend PHPUnit still fails at `DealSachDomainDatabaseTest::testSeedScenarioCoverage` (`Failed asserting that 0 is equal to 2 or is greater than 2.` at line 177) after fresh migrate+seed in Docker. T0025 scope changed only public catalog ordering and tests, so this suite failure was recorded rather than fixed out of scope. | Medium | Add a small backend ticket to trace and stabilize `DealSachDomainDatabaseTest::testSeedScenarioCoverage` against current seed data. |
