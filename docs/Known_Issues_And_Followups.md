...
|---|---:|---|---|---|---|---|
| KI-0016 | 2026-06-03 | No-ticket backend cleanup | Backend Admin routes | `GET /api/admin/reports` remains as an alias to `AdminController::dashboard` after T0029 removed the frontend reports route/page. Static review found backend tests and manual verification docs still reference the alias, so removal should be handled in a focused backend compatibility cleanup rather than bundled with test-baseline fixes. | Low | Verify current consumers, then either document the compatibility alias or remove `api/admin/reports` plus its backend tests/manual verification references. |
...
| KI-0015 | 2026-06-03 | Closed by no-ticket backend cleanup. Consolidated duplicate open KI-0015 rows and replaced the fixed stale-offer cutoff in `DealSachDomainDatabaseTest::countLatestStaleOffers()` with a dynamic cutoff derived from the latest seeded observation time minus 48 hours. Full backend PHPUnit now passes. |
| KI-0009 | 2026-06-04 | Closed by reviewer confirmation during T0032 known-issue cleanup. No code or seed changes were needed in this ticket. |
| KI-0014 | 2026-06-04 | Closed by reviewer confirmation during T0032 known-issue cleanup. No Docker Compose changes were made in this ticket. |
